...
var randIdMultiplier = Math.pow(10, 20);
var randId = function randId() {
  return Math.floor(Math.random() * randIdMultiplier).toString();
};
...
var mimickEvent = function mimickEvent(e) {
  var val = e.id + "," + JSON.stringify(e);
...
var EventEmitter = function () {
  function EventEmitter(room) {
    var opts = arguments.length <= 1 || arguments[1] === undefined ? {} : arguments[1];

    _classCallCheck(this, EventEmitter);

    if ((typeof room === "undefined" ? "undefined" : _typeof(room)) === "object") {
      opts = room;
      room = null;
    }
  ...
  _createClass(EventEmitter, [{
    ...
    value: function attach() {
      var origin = arguments.length <= 0 || arguments[0] === undefined ? window : arguments[0];
    ...
    value: function handleEvent(e) {
      var val = e.newValue;
      var c1 = val.indexOf(",");
      var room = val.slice(0, c1);

      if (e.key === "__events__" && room === this.room) {
        var c2 = val.indexOf(",", c1 + 1);
        var dataStr = val.slice(c2 + 1);
